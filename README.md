...
To get started:
 ...
 - Run `./rename_mysite_app my_new_app_name` to rename the 'mysite' app (and all references to it)
 - `python manage.py checksecure --settings=mysite.settings_live`
...
Create a Google App Engine project. Edit `app.yaml` and change `application: djangae-mysite` to `application: your-app-id`. Then, if you're in the `djangae-mysite` directory, run:
